...
namespace ShopVT.Controllers.Admin
{
    ...
    public class B10CustomerController : BaseController
    {
        ...
        private readonly string _table = "B10Customer";

        public B10CustomerController(IDataEdtitorService dataEdtitor, IDataExploreService explore, ILogger logger, IStorageService storageService, IMapper mapper)
        {
            _edit = dataEdtitor;
            _explore = explore;
            _logger = logger;
        ...
        public async Task<IActionResult> AddAsync([FromForm] B10CustomerModel addRequest)
        {
            try
            {
                ...
                var result = await _edit.AddRangeAsync<B10CustomerModel>(addRequest, _table, "CustomerCode", addRequest.Code, "Code", GetCurrentUserId());
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetById([FromRoute] int id)
        {
            try
            {
                var result = await _explore.GetDataByIdMultipleTable<B10CustomerModel>(_table, id,"Code","CustomerCode","",false, 1);
                return Ok(result);
            }
            catch (Exception ex)
            {
