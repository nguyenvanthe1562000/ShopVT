...
namespace ShopVT.Controllers.Admin
{
    ...
    public class B00PermissionController : BaseController
    {
        ...
        public B00PermissionController(IDataEdtitorService dataEdtitor, IDataExploreService explore, ILogger logger, IStorageService storageService, IMapper mapper)
        {
            _edit = dataEdtitor;
            _explore = explore;
            _logger = logger;
        ...
        public async Task<IActionResult> AddAsync([FromBody] PermissionRequest addRequest)
        {
            try
            {

                var user = await _explore.GetDataByIdOneTable<B00AppUserModel>("B00AppUser", addRequest.UserId, 1);
                foreach (var item in addRequest.PermissionFunctions)
                {
                    ...
                    if (item.Description.Equals(PermissionFunction.AccessApplication.ToString()))
                    {
                        {
                            ...
                            user.B00PermissionFunction_Json.Add(b00PermissionFunctionModel);
                        }
                    }

                //    user.B00PermissionFunction_Json.Add
        ...
        public async Task<IActionResult> GetById([FromRoute] int Userid)
        {
            ...
            {
                _logger.Log(LogType.Error, ex.Message, new StackTrace(ex, true).GetFrames().Last(), new { Userid = Userid });
                return StatusCode(StatusCodes.Status500InternalServerError, new ResponseMessageDto(MessageType.Error, ""));
            }
        }
        [HttpPost]
