...
{
    public class B00AppUserRepository : IB00AppUserRepository, IDisposable
    {
        ...
        public async Task<bool> Insert(B00AppUserModel model, int userId)
        {
            try
            {
                var result = await _dbHelper.ExecuteScalarSProcedureWithTransactionAsync("B00AppUser_create", "@code", model.code, "@username", model.username, "@PassWord", model.PassWord, "@FullName", model.FullName, "@IsActive", model.IsActive, "@EmployeeCode", model.EmployeeCode, "@user_id", userId);
        ...
        public async Task<bool> Update(B00AppUserModel model, int userId)
        {
            ...
            try
            {
                var result = await _dbHelper.ExecuteScalarSProcedureWithTransactionAsync("B00AppUser_update", "@code", model.code, "@username", model.username, "@PassWord", model.PassWord, "@FullName", model.FullName, "@IsActive", model.IsActive, "@EmployeeCode", model.EmployeeCode
                , "@user_id", userId);
                if (!string.IsNullOrEmpty(result.message.ToString()))
                {
                    return false;
