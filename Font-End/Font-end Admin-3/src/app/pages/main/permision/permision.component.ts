...
export class PermisionComponent extends BaseComponent implements OnInit {
  ...
  ShowViewDataGroup() {
    this._api.get(`${this.api}/group`).pipe(takeUntil(this.unsubscribe)).subscribe(res => {
      this.groupData = res;
    });
  }
  ...
  Add(form: NgForm, addDataIsGroup: boolean, addType: number) {
    ...
    this._api.postFormData(`${this.api}/add`, this.formData).pipe(takeUntil(this.unsubscribe)).subscribe(res => {
  ...
  Update(form: NgForm, addType: number) {
    ...
    this._api.putFormData(`${this.api}/update`, this.formData).pipe(takeUntil(this.unsubscribe)).subscribe(res => {
      this.Filter(this.filter);
      this.formData = new FormData();
      alert(res.messages[0].message)
    });
  ...
  Transfer() {
    ...
    this._api.put(`${this.api}/transfer`, this.selectItem).pipe(takeUntil(this.unsubscribe)).subscribe(res => {
      this.Filter(this.filter);
      alert(res.messages[0].message)
  ...
  Delete(id: number) {
    if (confirm("Bạn có chắc chắn muốn xóa?")) {
      this._api.deleteParamUrl(`${this.api}`, id).pipe(takeUntil(this.unsubscribe)).subscribe(res => {
        this.Filter(this.filter);
        alert(res.messages[0].message)
      });
    }
  ...
  Lookup(str: any) {
    this._api.getLookup(`employee`, str.query).pipe(takeUntil(this.unsubscribe)).subscribe(res => {
      this.lookupData = res;
    });
  }
  ...
  Restore(id: number) {
    if (confirm("Bạn có chắc chắn muốn khôi phục?")) {
      this._api.putParamUrl(`${this.api}/restore`, id).pipe(takeUntil(this.unsubscribe)).subscribe(res => {
        this.Filter(this.filter);
        alert(res.messages[0].message)
      });
    }
